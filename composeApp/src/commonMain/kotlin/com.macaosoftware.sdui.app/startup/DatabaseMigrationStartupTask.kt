...
class DatabaseMigrationStartupTask : StartupTask {
    ...
    override suspend fun initialize(koinComponent: KoinComponent): MacaoResult<Unit, DatabaseMigrationStartupTaskError> {
        return try {
            // todo: Remove this delay
            delay(1000)
            MacaoResult.Success(Unit)
        } catch (ex: Exception) {
            MacaoResult.Error(DatabaseMigrationStartupTaskError())
        }
    }
}

class DatabaseMigrationStartupTaskError : StartupTaskError()